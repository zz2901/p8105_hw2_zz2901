...
-   Data set `pols_month` after tidying contains `822` observations and
    ...
    -   Variable `president` was recoded from `prez_dem` and `prez_gop`.
        It indicates whether the president of the corresponding year is
        democratic or republican. Rest of variables start with `gov_`,
        `sen_`, \`rep\_\`\` indicates the number of governors, senators,
        or representatives for each party in that year.

-   Data set `snp` contains `date` and `close` variables. After tidying
    and seperating `date` into `year` and `month` variables, it has
    `787` observations and `3` variables.

    -   Variable `close` indicates the closing values of the S&P stock
...
-   Data set `unemployment` originally contains `Year` and percentage of
    ...
    -   Variable `month` stores the original column names, which
        represents months.
    -   Variable `unemployment_p` stores the row values for each month’s
        unemployment precentages.
